...
Skills live in `bangerpdf/skills_data/<skill-name>/SKILL.md` and ship inside
the wheel as package data. They get installed into the user's Claude Code
skills directory (default: `~/.claude/skills/`) where Claude Code auto-
discovers them.

Adapted from n8n-cli's skills.py (same pattern, same install semantics).
"""
...
SKILLS_PACKAGE = "bangerpdf.skills_data"
...
def cmd_list(as_json: bool = False) -> None:
    ...
    for r in rows:
        ...
        print(f"  {marker}  {r['name']:<{name_width}}  {desc}")
    print()
    print("Install all:        bangerpdf skills install")
    print("Install one:        bangerpdf skills install <name>")
    print("Force overwrite:    bangerpdf skills install --force")
...
def cmd_install(
    ...
    if not names:
        ...
        for n in names:
            if n not in bundled:
                msg = f"Unknown skill: {n}. Run `bangerpdf skills list` to see available."
                if as_json:
    ...
    for name in to_install:
        ...
        if already and not force:
            results.append({"name": name, "status": "skipped", "reason": "already installed (use --force)"})
            continue
...
def cmd_uninstall(names: Iterable[str], as_json: bool = False) -> None:
    ...
    for name in names:
        ...
        if not (skill_dir / SKILL_FILENAME).is_file():
            results.append({"name": name, "status": "skipped", "reason": "no SKILL.md found"})
            continue
